...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        if let Some(index) = self
            .chunks()
            .iter()
            .position(|i| i.chunk_type().bytes() == chunk_type.as_bytes())
            {
                return Ok(self.chunks.remove(index));
            }
        

        Err("ChunkType not found".into())
    }
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        return Png::STANDARD_HEADER
            .iter()
            .copied()
            .chain(self.chunks.iter().flat_map(|x| x.as_bytes()))
            .collect();
    }
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Png> {
        ...
        //compare it to the Png::STANDARD_HEADER
        if header == Png::STANDARD_HEADER {
            let mut chunks: Vec<Chunk> = Vec::new();
            let mut i :usize= 8;

            while i < bytes.len() {
                let length = (u32::from_be_bytes(bytes[i..i+4].try_into()?) + 12) as usize;

                match Chunk::try_from(&bytes[i..i+length]) {
                    Ok(chunk) => {
                        chunks.push(chunk);
                        i += length;
                    }
                    Err(_) => {
                        panic!("Chunk Error in Png");
                    }
                }
            }

            return Ok(Png{header:Png::STANDARD_HEADER, chunks})
        } else {
            return Err("".into())
        }
    }
